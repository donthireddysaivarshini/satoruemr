...
gm_items = []
for row_idx in range(130, 170):  # GM items start at row 130
    relevant = ws.cell(row=row_idx, column=7).value
    name = ws.cell(row=row_idx, column=3).value
    local_no = row_idx - 129  # GM local number 1 onwards (GM94 = local 1)
    if name or relevant:
        ...
        # Item is always visible if age is None
        always_visible = (age_threshold is None)
        gm_items.append((local_no, name, age_threshold, always_visible))
        # Print full relevant expression
        print(f'GM{local_no}: relevant={rel_str}')
        print(f'  -> age_threshold={age_threshold}, always_visible={always_visible}')
        print()
...
for local_no, name, age_threshold, always_visible in gm_items:
    if always_visible:
        ...
        visible_at_9m.append(local_no)
    # Also check: if show_all='yes', all items visible
...
if actual == expected:
    ...
    print('\\nDetails:')
    for local_no in range(1, 13):  # Check GM1-GM12
